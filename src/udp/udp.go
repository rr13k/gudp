...
type Server struct {
	protocolVersion     [2]byte       // 协议版本(预留字段)
...
func (s *Server) handleRecord(record []byte, addr *net.UDPAddr) {
	...
	// var chat = protoc.ApiChat{}
	// proto.Unmarshal(record, &chat)

	// switch chat.NoticeWay.(type) {
	// case *protoc.ApiChat_Email:
	// 	fmt.Println("接收到email")
	// case *protoc.ApiChat_Phone:
	// 	fmt.Println("接收到phone")

	// }
	// return

	var apiMsg = protoc.ApiMessage{}
	err := proto.Unmarshal(record, &apiMsg)
	...
	switch apiMsg.NoticeWay.(type) {
	case *protoc.ApiMessage_Ping:
		fmt.Println("接收ping消息")
		_pingMessage := apiMsg.GetPing()
		s.handlePingRecord(context.Background(), addr, _pingMessage)

	case *protoc.ApiMessage_Pong:
		fmt.Println("asdad")

	case *protoc.ApiMessage_Mytest:
		fmt.Println("asdadttt")

	case *protoc.ApiMessage_HandshakeMessage:
		fmt.Println("处理握手")
		_handshakeMessage := apiMsg.GetHandshakeMessage()
		s.handleHandshakeRecord(context.Background(), addr, _handshakeMessage)
...
func (s *Server) handleHandshakeRecord(ctx context.Context, addr *net.UDPAddr, r *protoc.HandshakeMessage) {
	// var payload []byte
	...
	if len(r.GetCookie()) == 0 {
		...
		serverHandshakeVerify := &protoc.HandshakeMessage{}
		serverHandshakeVerify.Cookie = cookie
		serverHandshakeVerify.Timestamp = time.Now().UnixNano() / int64(time.Millisecond)

		handshakePayload, err := proto.Marshal(serverHandshakeVerify)
		if err != nil {
		...
		err = s.sendToClient(cl, serverHandshakeHelloBytes)
		if err != nil {
...
func (s *Server) handlePingRecord(ctx context.Context, addr *net.UDPAddr, r *protoc.Ping) {
	cl, ok := s.findClientByAddr(addr)
	if !ok {
...
func StartUdpServer(host string, port int) *net.UDPConn {
	// 设置服务器监听地址和端口
